...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        caption_embed_pos = self.positional_encoding(caption_embed)
        features_D = self.visual_projection(features).unsqueeze(1)

        tgt_mask=torch.tril(torch.ones(T,T,device=captions.device))

        scores=self.transformer(caption_embed_pos,features_D,tgt_mask=tgt_mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        ima_embed=self.patch_embed(x)
        ima_embed=self.positional_encoding(ima_embed)

        x=self.transformer(ima_embed)
