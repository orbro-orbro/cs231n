...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        div_term=torch.pow(10000,-torch.arange(0,embed_dim,2,dtype=torch.float32)/embed_dim)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        atten_scores=torch.matmul(Q,K.transpose(-2,-1))/math.sqrt(E//H)
        if attn_mask is not None:
            atten_scores.masked_fill_(attn_mask==0, float('-inf'))
        atten_scores=self.attn_drop(torch.softmax(atten_scores,dim=-1))
        atten_scores=torch.matmul(atten_scores,V)
        atten_scores = atten_scores.transpose(1, 2).contiguous().view(N, S,E)
        output=self.proj(atten_scores)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return output
