...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    processed_images = [
        clip_preprocess(Image.fromarray(image)).unsqueeze(0) for image in images
    ]
    images_tensor = torch.cat(processed_images, dim=0).to(device)
    image_features = clip_model.encode_image(images_tensor)
    similarity = get_similarity_no_loop(image_features, text_features)
    class_indices = similarity.argmax(dim=1)
    pred_classes = [class_texts[index.item()] for index in class_indices]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        # (1, D) @ (D, N) -> (1, N)
        similarities = (
            text_features @ self.image_features.T
        )
