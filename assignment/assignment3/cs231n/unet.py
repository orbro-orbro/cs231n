...
class Unet(nn.Module):
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            ##################################################################
            resnet1=ResnetBlock(dim_in,dim_in,context_dim)
            resnet2=ResnetBlock(dim_in,dim_in,context_dim)
            down_sample=Downsample(dim_in,dim_out=dim_out)
            down_block.append(resnet1)
            down_block.append(resnet2)
            down_block.append(down_sample)
            ##################################################################
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out_ups):
            ...
            ##################################################################
            up_sample=Upsample(dim_in,dim_out=dim_out)
            resnet1=ResnetBlock(2*dim_out,dim_out,context_dim)
            resnet2=ResnetBlock(2*dim_out,dim_out,context_dim)
            up_block.append(up_sample)
            up_block.append(resnet1)
            up_block.append(resnet2)
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        cond_out = self.forward(x, time, model_kwargs=model_kwargs)
        uncond_kwargs = copy.deepcopy(model_kwargs)
        uncond_kwargs["text_emb"] = None
        uncond_out = self.forward(x, time, model_kwargs=uncond_kwargs)
