...
def sim(z_i, z_j):
    ...
    ##############################################################################
    norm_dot_product = torch.matmul(z_i,z_j.T)/(torch.linalg.norm(z_i)*torch.linalg.norm(z_j))
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2*N):
            if i != k:
                fenmu1 += torch.exp(sim(z_k,out[i])/tau)
            if i != k+N:
                fenmu2 += torch.exp(sim(z_k_N,out[i])/tau)
        fenzi = torch.exp(sim(z_k,z_k_N)/tau)
        loss1 = -torch.log(fenzi / fenmu1)
        loss2 = -torch.log(fenzi / fenmu2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = pos_pairs/(torch.linalg.norm(out_left,dim=1,keepdim=True)*torch.linalg.norm(out_right,dim=1,keepdim=True))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    pos_dim = sim_positive_pairs(out_left,out_right)
    pos_dim = torch.cat([pos_dim,pos_dim],dim=0)  
    # Step 3: Compute the numerator value for all augmented samples.
    numerator = None
    numerator = torch.exp(pos_dim/tau)
