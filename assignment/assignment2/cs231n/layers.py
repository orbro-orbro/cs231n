...
def softmax_loss(x, y):
    ...
    correct=p[np.arange(x.shape[0]),y]
    loss =-np.mean(np.log(correct))
    dscores=p.copy()
    dscores[np.arange(x.shape[0]),y]=dscores[np.arange(x.shape[0]),y]-1
...
def batchnorm_backward(dout, cache):
    ...
    inv_std=1/np.sqrt(sample_var+eps)
    dx_centered_direct = dx_hat * inv_std
    dinv_std = np.sum(dx_hat * x_centered, axis=0)
    dvar = dinv_std * (-0.5) * (sample_var + eps) ** (-1.5)
    dx_centered_var = dvar * 2.0 * x_centered / x.shape[0]
    dx_centered = dx_centered_direct + dx_centered_var
    dmean = -np.sum(dx_centered, axis=0)
    dx = dx_centered + dmean / x.shape[0]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    cache = (x_hat, sample_var, gamma, eps)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def layernorm_backward(dout, cache):
    ...
    # still apply!                                                            #
    ###########################################################################
    x_hat, sample_var, gamma, eps = cache
    D = dout.shape[1]
        ...
        for f in range(w.shape[0]):
            for i in range(0,H_out):
                for j in range(0,W_out):
                    ...
                    w_start=j*stride
                    w_end=w_start+WW
                    windows=x_pad[batch,:,h_start:h_end,w_start:w_end]
                    out[batch,f,i,j]=np.sum(windows*w[f])+b[f]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    window=x[n,c,i*stride:i*stride+pool_h,j*stride:j*stride+pool_w]
